...
app.post("/addRecipe", async (req, res) => {
    try {
        ...
        const existingDoc = await db.collection("recipe").findOne({ "id": newDocument.id });
        if (existingDoc) {
            return res.status(409).send({ error: 'Conflict: a recipe with this ID already exists' });
        }
...
app.put("/updateRecipe/:id", async (req, res) => {
    ...
    const options = { };
    const results = await db.collection("recipe").updateOne(query, updateData, options);
    res.status(200);
    res.send(results);
    });
